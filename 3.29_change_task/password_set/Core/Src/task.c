...
#define CURXY (temp.x + ((5 - temp.y) * 6))
#define TESTXY(X, Y) ((X) + ((5 - (Y)) * 6))
#define init_value firF = sel = ModeF = temp.x = temp.y = joy_result = 0
...
const char ptWorkCate[4][5] = { "SAVE", "USE", "FIND", "MKPT" };
const char keyboard_small[4][11] = { "1234567890", "qwertyuiop", "asdfghjkl;", "zxcvbnm,. " };
const char keyboard_big[4][11] = { "!@#$%^&*()", "QWERTYUOIP", "ASDFGHJKL:", "ZXCVBNM<> " };
...
uint8_t firF = 1, sel, ModeF, buzM, ledM, ptionC, findC, start_check, use_num, password_num;
